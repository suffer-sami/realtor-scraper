// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.27.0
// source: sales_data.sql

package database

import (
	"context"
	"database/sql"
)

const createSalesData = `-- name: CreateSalesData :one
INSERT INTO sales_data (count, min, max, last_sold_date, agent_id)
VALUES (
    ?,
    ?,
    ?,
    ?,
    ?
)
ON CONFLICT (agent_id) 
DO UPDATE SET
    count = EXCLUDED.count,
    min = EXCLUDED.min,
    max = EXCLUDED.max,
    last_sold_date = EXCLUDED.last_sold_date
RETURNING id, count, min, max, agent_id, last_sold_date
`

type CreateSalesDataParams struct {
	Count        sql.NullInt64
	Min          sql.NullInt64
	Max          sql.NullInt64
	LastSoldDate sql.NullTime
	AgentID      sql.NullString
}

func (q *Queries) CreateSalesData(ctx context.Context, arg CreateSalesDataParams) (SalesDatum, error) {
	row := q.db.QueryRowContext(ctx, createSalesData,
		arg.Count,
		arg.Min,
		arg.Max,
		arg.LastSoldDate,
		arg.AgentID,
	)
	var i SalesDatum
	err := row.Scan(
		&i.ID,
		&i.Count,
		&i.Min,
		&i.Max,
		&i.AgentID,
		&i.LastSoldDate,
	)
	return i, err
}
